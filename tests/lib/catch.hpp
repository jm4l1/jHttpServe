...
constexpr auto operator"" _catch_sr(char const* rawChars, std::size_t size) noexcept -> Catch::StringRef
{
	...
	#define INTERNAL_CATCH_TYPE_GEN                                                                                                    \
		...
		{                                                                                                                              \
			using type =                                                                                                               \
				typename append<TypeList<Container<Elems...>>, typename rewrap<TemplateTypeList<Container>, Elements...>::type>::type; \
		};                                                                                                                             \
                                                                                                                                       \
			...
			{                                                                                                                           \
				...
				{                                                                                                                       \
					...
					{                                                                                                                   \
						...
						using expander = int[];                                                                                         \
						(void)expander{                                                                                                 \
							(reg_test(Types{}, Catch::NameAndTags{ Name " - " + std::string(tmpl_types[index]), Tags }), index++, 0)... \
						}; /* NOLINT */                                                                                                 \
					}                                                                                                                   \
				};                                                                                                                      \
			...
			{                                                                                                                           \
				...
				{                                                                                                                       \
					...
					{                                                                                                                   \
						...
						constexpr char const* types_list[] = { CATCH_REC_LIST(INTERNAL_CATCH_STRINGIZE_WITHOUT_PARENS,                  \
			...
			{                                                                                                          \
				...
				{                                                                                                      \
					static const auto& enumInfo =                                                                      \
						::Catch::getMutableRegistryHub().getMutableEnumValuesRegistry().registerEnum(#enumName,        \
																									 #__VA_ARGS__,     \
																									 { __VA_ARGS__ }); \
...
namespace Catch
{
	...
	// end catch_message.h
		...
		#define INTERNAL_CATCH_TEST(macroName, resultDisposition, ...)                               \
			...
			} while ((void)0, (false) && static_cast<bool>(!!(__VA_ARGS__)))  // the expression here is never evaluated at runtime but it
	...
	class ExceptionTranslatorRegistrar
	{
		...
		public:
			...
			std::string translate(ExceptionTranslators::const_iterator it, ExceptionTranslators::const_iterator itEnd) const override
			{
				try
				{
					if (it == itEnd) std::rethrow_exception(std::current_exception());
					else
...
namespace Catch
{
	...
	public:
		...
		void streamReconstructedExpression(std::ostream& os) const override
		{
			...
			if (matcherAsString == Detail::unprintableString) os << m_matcherString;
			else
				os << matcherAsString;
		}
	};
...
namespace Catch
{
	...
	TestCase
		makeTestCase(ITestInvoker* testCase, std::string const& className, NameAndTags const& nameAndTags, SourceLineInfo const& lineInfo);
}  // namespace Catch
			...
			Class cls = classes[c];
			{
				...
				for (u_int m = 0; m < count; m++)
				{
					...
					if (startsWith(methodName, "Catch_TestCase_"))
					{
						...
						getMutableRegistryHub().registerTest(makeTestCase(new OcMethod(cls, selector),
																		  className,
...
namespace Catch
{
	...
	{
		std::string expr;
		if (m_info.macroName.empty()) expr = static_cast<std::string>(m_info.capturedExpression);
		else
		{
					...
					auto operator*() const -> std::string
					{
						...
						for (size_t i = 0; i < m_columns.size(); ++i)
						{
							...
							if (m_iterators[i] != m_columns[i].end())
							{
								...
								row += padding + col;
								if (col.size() < width) padding = std::string(width - col.size(), ' ');
								else
									padding = "";
							}
							else
...
namespace Catch
{
	namespace clara
	{
		namespace detail
		{
			...
			inline auto convertInto(std::string const& source, T& target) -> ParserResult
			{
				...
				ss << source;
				ss >> target;
				if (ss.fail()) return ParserResult::runtimeError("Unable to convert '" + source + "' to destination type");
				else
			...
			inline auto convertInto(std::string const& source, bool& target) -> ParserResult
			{
				...
				if (srcLC == "y" || srcLC == "1" || srcLC == "true" || srcLC == "yes" || srcLC == "on") target = true;
	...
	#ifdef CATCH_PLATFORM_WINDOWS
						if (name[0] != '-' && name[0] != '/') return Result::logicError("Option name must begin with '-' or '/'");
	#else
...
namespace Catch
{
	...
	clara::Parser makeCommandLineParser(ConfigData& config)
	{
		...
		auto const setWarning = [&](std::string const& warning)
		{
			...
			if (warningSet == WarnAbout::Nothing) return ParserResult::runtimeError("Unrecognised warning: '" + warning + "'");
		...
		{
			if (startsWith("declared", order)) config.runOrder = RunTests::InDeclarationOrder;
		...
		auto const setColourUsage = [&](std::string const& useColour)
		{
			...
			if (mode == "yes") config.useColour = UseColour::Yes;
			else if (mode == "no")
		...
		auto const setWaitForKeypress = [&](std::string const& keypress)
		{
			...
			if (keypressLc == "start") config.waitForKeypress = WaitForKeypress::BeforeStart;
		...
		auto const setReporter = [&](std::string const& reporter)
		{
			...
			if (factories.end() != result) config.reporterName = lcReporter;
			else
...
namespace Catch
{
	namespace
	{
		...
		IColourImpl* platformColourInstance()
		{
			...
			IConfigPtr config = getCurrentContext().getConfig();
			UseColour::YesOrNo colourMode = config ? config->useColour() : UseColour::Auto;
			if (colourMode == UseColour::Auto) colourMode = useColourOnPlatform() ? UseColour::Yes : UseColour::No;
...
namespace Catch
{
	...
	std::size_t listTestsNamesOnly(Config const& config)
	{
		...
		{
			matchedTests++;
			if (startsWith(testCaseInfo.name, '#')) Catch::cout() << '"' << testCaseInfo.name << '"';
			else
	...
	std::size_t listTags(Config const& config)
	{
		...
		for (auto const& testCase : matchedTestCases)
		{
			for (auto const& tagName : testCase.getTestCaseInfo().tags)
			{
				...
				if (countIt == tagCounts.end()) countIt = tagCounts.insert(std::make_pair(lcaseTagName, TagInfo())).first;
	...
	Option<std::size_t> list(std::shared_ptr<Config> const& config)
	{
		...
		if (config->listTestNamesOnly()) listedCount = listedCount.valueOr(0) + listTestsNamesOnly(*config);
...
namespace Catch
{
	...
	void RunContext::assertionEnded(AssertionResult const& result)
	{
		...
		else if (!result.isOk())
		{
			...
			if (m_activeTestCase->getTestCaseInfo().okToFail()) m_totals.assertions.failedButOk++;
			else
	...
	{
		if (m_unfinishedSections.empty()) m_activeSections.back()->fail();
		else
			m_activeSections.back()->close();
		m_activeSections.pop_back();
	...
	IResultCapture& getResultCapture()
	{
		if (auto* capture = getCurrentContext().getResultCapture()) return *capture;
		else
...
namespace Catch
{
	...
	Section::~Section()
	{
		if (m_sectionIncluded)
		{
			...
			if (uncaught_exceptions()) getResultCapture().sectionEndedEarly(endInfo);
			else
...
namespace Catch
{
	...
	int Session::runInternal()
	{
		...
		CATCH_TRY
		{
			...
			// Handle list request
			if (Option<std::size_t> listed = list(m_config)) return static_cast<int>(*listed);
...
namespace Catch
{
	...
	auto makeStream(StringRef const& filename) -> IStream const*
	{
		if (filename.empty()) return new Detail::CoutStream();
		else if (filename[0] == '%')
		{
			if (filename == "%debug") return new Detail::DebugOutStream();
			else
...
namespace Catch
{
	...
	namespace
	{
		...
		{
			if (startsWith(tag, '.') || tag == "!hide") return TestCaseInfo::IsHidden;
			else if (tag == "!throws")
		...
		for (char c : nameAndTags.tags)
		{
			...
			else
			{
				if (c == ']')
				{
					...
					if ((prop & TestCaseInfo::IsHidden) != 0) isHidden = true;
...
namespace Catch
{
	...
	void XmlEncode::encodeTo(std::ostream& os) const
	{
		...
		for (std::size_t idx = 0; idx < m_str.size(); ++idx)
		{
			...
			case '>':
				// See: http://www.w3.org/TR/xml/#syntax
				if (idx > 2 && m_str[idx - 1] == ']' && m_str[idx - 2] == ']') os << "&gt;";
				else
					os << c;
				break;

			case '\"':
				if (m_forWhat == ForAttributes) os << "&quot;";
				else
					os << c;
				break;
		...
		public:
			...
			void print()
			{
				...
				case ResultWas::ExpressionFailed:
					if (result.isOk()) printResultType(Colour::ResultSuccess, failedString() + std::string(" - but was ok"));
					else
...
namespace Catch
{
	...
	public:
		...
		friend TablePrinter& operator<<(TablePrinter& tp, ColumnBreak)
		{
			...
			if (colInfo.justification == ColumnInfo::Left) tp.m_os << colStr << padding << ' ';
			else
	...
	void ConsoleReporter::sectionEnded(SectionStats const& _sectionStats)
	{
		...
		if (_sectionStats.missingAssertions)
		{
			lazyPrint();
			Colour colour(Colour::ResultError);
			if (m_sectionStack.size() > 1) stream << "\nNo assertions in section";
			else
	...
	void ConsoleReporter::printTotalsDivider(Totals const& totals)
	{
		if (totals.testCases.total() > 0)
		{
			...
			if (totals.testCases.allPassed()) stream << Colour(Colour::ResultSuccess) << std::string(passedRatio, '=');
			else
...
namespace Catch
{
	...
	void JunitReporter::writeGroup(TestGroupNode const& groupNode, double suiteTime)
	{
		...
		if (m_config->showDurations() == ShowDurations::Never) xml.writeAttribute("time", "");
		else
	...
	void JunitReporter::writeSection(std::string const& className, std::string const& rootName, SectionNode const& sectionNode)
	{
		...
		for (auto const& childNode : sectionNode.childSections)
			if (className.empty()) writeSection(name, "", *childNode);
			else
...
namespace Catch
{
	...
	void XmlReporter::testRunStarting(TestRunInfo const& testInfo)
	{
		...
		if (m_config->testSpec().hasFilters()) m_xml.writeAttribute("filters", serializeFilters(m_config->getTestsOrTags()));
	...
	void XmlReporter::testCaseEnded(TestCaseStats const& testCaseStats)
	{
		...
		if (m_config->showDurations() == ShowDurations::Always) e.writeAttribute("durationInSeconds", m_testCaseTimer.getElapsedSeconds());

		if (!testCaseStats.stdOut.empty()) m_xml.scopedElement("StdOut").writeText(trim(testCaseStats.stdOut), XmlFormatting::Newline);
		if (!testCaseStats.stdErr.empty()) m_xml.scopedElement("StdErr").writeText(trim(testCaseStats.stdErr), XmlFormatting::Newline);
